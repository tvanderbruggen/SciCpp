...
TEST_CASE("Physical constants") {
    ...
    REQUIRE(almost_equal<2>(1E12 * phys_cst::epsilon0,
                            8.854187818840058860_F / 1_m));
    ...
    REQUIRE(almost_equal<2>(1E24 * phys_cst::muB,
                            9.274010065738068515475452810387_J / 1_T));
    // printf("%.30Le\n", phys_cst_l::muN.value());
    REQUIRE(almost_equal<2>(1E27 * phys_cst::muN,
                            5.050783739271897144306250269724_J / 1_T));
    // printf("%.30Le\n", phys_cst_l::Rinf.value());
    REQUIRE(almost_equal<2>(phys_cst::Rinf,
                            10973731.56803849312655074754730 / 1_m));
    // printf("%.30Le\n", phys_cst_l::a0.value());
    REQUIRE(almost_equal<2>(1E11 * phys_cst::a0,
                            5.291772105467402030742718729114_m));
}
